"""
    ...
    This script is a custom fork.
    URL of original: https://pypi.python.org/pypi/python-ping/
...
class Ping(object):
    def __init__(self, destination, timeout=1000, packet_size=32, own_id=None):
    ...
    def print_start(self):
        print("PING {0} [{1}]: with {2} bytes of data:".format(self.destination, self.dest_ip, self.packet_size))

    def print_unknown_host(self, e):
        print("Unknown host: {0} ({1})".format(self.destination, e))

    def print_success(self, delay, ip, packet_size, ip_header, icmp_header):
        if ip == self.destination:
            from_info = ip
        else:
            from_info = "{0} [{1}]".format(self.destination, ip)

        print("{0} bytes from {1}: icmp_seq={2} ttl={3} time={4:.1f} ms".format(
    ...
    def print_exit(self):
        print("--- {0} ping statistics ---".format(self.destination))
        ...
        if self.receive_count > 0:
            print("round-trip min/avg/max = {0:.1f}/{1:.1f}/{2:.1f} ms".format(
                int(self.min_time), int(self.total_time / self.receive_count), int(self.max_time)))
    ...
    def signal_handler(self, signum, frame):
        ...
        """
        self.print_exit()
        print("^Terminated with signal {0}\n".format(signum))
        sys.exit(0)
    ...
    def do(self):
        ...
        """
        # icmp_proto_id = socket.getprotobyname("icmp")
        # hardcoded until: https://tracker.crystax.net/issues/1164
        icmp_proto_id = 1
        current_socket = socket.socket(socket.AF_INET, socket.SOCK_RAW, icmp_proto_id)
    ...
    def send_one_ping(self, current_socket):
        ...
        for i in range(start_val, start_val + (self.packet_size)):
            data[i - start_val] = (i & 0xff)  # Keep chars in the 0-255 range
        ...
        try:
            ...
            print("General failure ({0})".format(e))
            current_socket.close()
            return
...
def verbose_ping(hostname, timeout=1000, count=4):
